...
const authController = {
  register: async (req, res) => {
    try {
      ...
      if (!contactPhone) {
        return res.status(400).json({ msg: "Please enter contact phone." });
      }

      if (!city) {
        return res.status(400).json({ msg: "Please enter City." });
      }

      if (postalCode.length > 5 || postalCode.length < 5) {
        return res.status(400).json({ msg: "Postal code must have 5 numbers." });
      }
